...
public class Controller_Cart_Item_Component implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        moveToWishlistButton.setOnAction(e -> handleMoveToWishlist());
    ...
    private void handleMoveToWishlist() {
        if (product != null && userEmail != null) {
            boolean moved = productStack.moveFromCartToFavorites(product.getIdProduct(), userEmail);
            if (moved) {
                showAlert(Alert.AlertType.INFORMATION, "Éxito", "Producto movido a la lista de deseos.");
                if (onRemoveCallback != null) {
                    onRemoveCallback.run();
                }
            } else {
                showAlert(Alert.AlertType.WARNING, "Advertencia", "No se pudo mover el producto a la lista de deseos.");
            }
        }
    }
    ...
    private void handleRemove() {
        if (product != null && userEmail != null) {
            boolean removed = productStack.removeFromCart(product.getIdProduct(), userEmail);
            if (removed) {
                showAlert(Alert.AlertType.INFORMATION, "Éxito", "Producto eliminado del carrito.");
                if (onRemoveCallback != null) {
                    onRemoveCallback.run();
                }
            } else {
